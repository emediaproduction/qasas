...
class _QasasPlayerState extends State<QasasPlayer>
  ...
  late ConcatenatingAudioSource _playlist; // Declare as late
  bool isPlaying = true;
  ...
  Future<void> _initPlaylist() async {
    List<AudioSource> sources = List.generate(widget.musicUrls.length, (i) {
      return AudioSource.uri(
        Uri.parse(widget.musicUrls[i]),
        tag: MediaItem(
          id: "${i + 1}",
          title: widget.musicTitles[i],
          artUri: Uri.parse(widget.playlistImage),
        ),
      );
    });
